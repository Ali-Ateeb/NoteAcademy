        ...
        cur.execute("""
            select qa.question_id, qa.storage_key, qa.page_number, qa.bbox,
             ...
             where q.extraction_status in ('approved', 'needs_review', 'extracted')
               -- Only a question's own crop. A mark-scheme row picture has a row of
               -- its own here too, but it is rendered from the mark scheme PDF, not
               -- the question paper this function re-renders from; recovering it
               -- from the wrong document would upload the wrong image. Those are
               -- regenerated by `mark-scheme-crops`.
